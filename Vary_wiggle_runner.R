...
if(length(Args)==3){
  ...
  myID = paste(c("WIG", method, rho, BOseed), collapse = "_")
  ...
  else if (grepl("Book", CPU)) setwd("/Volumes/DataStorage/Dropbox/PhD/CRN/CRN")
...
for (i in BOseed + 1000*(0:9)){
  ...
  # Make test data
  set.seed(i)
  TruePars = c(10, 100^2, 5, rho*NOISE, 0, 0.0001 + (1-rho)*NOISE)
  Nseeds   = 115
  X_domain = 1:100
...
if(F){
  ...
  if (CPU=="akademikkunoMacBook-ea.local"){
    system("rsync -r -v /Volumes/DataStorage/Dropbox/PhD/CRN/CRN/ godzilla:/storage/maths/phrnaj/PhD/CRN/")
  }
  
  NrunFileArray("/storage/maths/phrnaj/PhD/CRN/Vary_wiggle_runner.R", 3300)
