...
Y_wiggles = t(mvrnorm(60, rep(0, length(X_f)), SEkernelw(X_f,X_f)))
Y_wiggles = Y_wiggles + matrix(rnorm(60*100, 0, sqrt(TruePars[6])), 100, 60)
Y_wiggles = cbind(0, Y_wiggles)

Y_Offsets = c(0, rnorm(60, 0, sqrt(TruePars[5])))

TestFunY  = Y_f + Y_wiggles + matrix(Y_Offsets, 100, 61, byrow=T)
TestFun = function(xs){
  xs = matrix(xs, ncol=2)
